...
def reconstruct_metrics(activations,down_weight,indices,bias=None):
 activations=activations.to(device=down_weight.device,dtype=down_weight.dtype);bias=None if bias is None else bias.to(device=down_weight.device,dtype=down_weight.dtype);sparse=torch.zeros_like(activations).scatter_(-1,indices.to(activations.device),activations.gather(-1,indices.to(activations.device)));dense=torch.nn.functional.linear(activations,down_weight,bias);pred=torch.nn.functional.linear(sparse,down_weight,bias);return output_metrics(dense,pred)
