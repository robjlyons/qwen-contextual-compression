...
def main(argv=None):
 p=argparse.ArgumentParser();p.add_argument("--model",default="Qwen/Qwen3.8-27B");p.add_argument("--input",type=Path);p.add_argument("--dataset");p.add_argument("--dataset-split",default="train");p.add_argument("--text-field",default="text");p.add_argument("--category-field",default="category");p.add_argument("--exclude-capture-metadata",type=Path);p.add_argument("--schedules",default=DEFAULT_ORDER);p.add_argument("--schedule-config",type=Path,default=Path("configs/end_to_end_schedules.yaml"));p.add_argument("--schedule-interpolation",choices=["nearest","linear"]);p.add_argument("--max-eval-tokens",type=int,default=250);p.add_argument("--max-prompt-tokens",type=int,default=128);p.add_argument("--device-map",default="auto");p.add_argument("--offload-folder");p.add_argument("--cache-dir");p.add_argument("--dtype",default="bfloat16");p.add_argument("--revision");p.add_argument("--norm-chunk-columns",type=int,default=256);p.add_argument("--gpu-headroom-mib",type=int);p.add_argument("--cpu-max-memory-gib",type=float);p.add_argument("--output-dir",type=Path,default=Path("results/end_to_end"));p.add_argument("--force",action="store_true");p.add_argument("--generation",action="store_true");p.add_argument("--generation-prompts",type=int,default=20);p.add_argument("--max-new-tokens",type=int,default=64);a,_=p.parse_known_args(argv)
 if a.norm_chunk_columns<=0:p.error("--norm-chunk-columns must be positive")
 if a.max_eval_tokens<=0 or a.max_prompt_tokens<2:p.error("token limits require --max-eval-tokens > 0 and --max-prompt-tokens >= 2")
 if a.cpu_max_memory_gib is not None and not a.offload_folder:p.error("--cpu-max-memory-gib requires --offload-folder so overflow weights can use disk")
 ...
 remaining=a.max_eval_tokens;estimated_prompts=0
 for item in corpus:
  available=max(0,min(len(tokenizer(item["text"])["input_ids"]),a.max_prompt_tokens)-1);remaining-=min(available,remaining);estimated_prompts+=1
  if remaining<=0:break
 preflight={"input_records":len(corpus),"approx_corpus_tokens":diagnostics["raw_tokens"],"max_eval_tokens":a.max_eval_tokens,"max_prompt_tokens":a.max_prompt_tokens,"estimated_prompts_required":estimated_prompts,"schedules":names,"evaluation_strategy":"streaming-per-prompt","host_rss_before_model_load_mib":psutil.Process(os.getpid()).memory_info().rss/2**20};print(json.dumps(preflight,indent=2))
 ...
 model=load_model(a.model,a.dtype,a.device_map,a.offload_folder,a.revision,full_model=True,cache_dir=a.cache_dir,max_memory=max_memory);model_rss=psutil.Process(os.getpid()).memory_info().rss/2**20;(a.output_dir/"model_metadata.json").write_text(json.dumps({"model":a.model,"revision":a.revision,"class":type(model).__name__,"layers":layer_count,"dtype":a.dtype,"device_map":getattr(model,"hf_device_map",None),"max_memory":max_memory,"host_rss_before_model_load_mib":preflight["host_rss_before_model_load_mib"],"host_rss_after_model_load_mib":model_rss},default=str,indent=2)+"\n");print(json.dumps(evaluate_schedules(model,tokenizer,corpus,schedules,a.output_dir,a.max_eval_tokens,a.force,a.norm_chunk_columns,a.max_prompt_tokens),indent=2))
