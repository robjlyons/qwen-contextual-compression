...
from evaluation.metrics import stable_cosine
...
def _logit_metrics_chunk(dense:Tensor,sparse:Tensor)->dict[str,Tensor]:
    ...
    return {"logit_cosine":stable_cosine(d,s,dim=-1),"logit_relative_l2":torch.linalg.vector_norm(delta,dim=-1)/torch.linalg.vector_norm(d,dim=-1).clamp_min(1e-12),"logit_mse":delta.square().mean(-1),"max_absolute_logit_difference":delta.abs().amax(-1),
