...
from predictor.models import StaticHot,create_predictor
...
def prepare_evaluation(target_dir:Path,run_dir:Path,device="cpu"):
 ...
 data=torch.load(target_path,map_location="cpu",weights_only=True);checkpoint=torch.load(checkpoint_path,map_location="cpu",weights_only=False);split=json.loads(split_path.read_text());config=checkpoint["config"];model=create_predictor(config["kind"],data["inputs"].shape[1],data["scores"].shape[1],config["latent_dim"]);model.load_state_dict(checkpoint["model"],strict=True);model=model.to(device).eval()
...
def evaluate(target_dir:Path,run_dir:Path,retentions,device="cpu"):
 data,model,t,diagnostics=prepare_evaluation(target_dir,run_dir,device);rows=[];samples=[];selections={}
 with torch.inference_mode():
  pred=model(t["x"])
  for r in retentions:
   for method,ranking in (("predictor",pred),("oracle",t["target"]),("static",t["static"].expand_as(t["target"]))):
    m=selection_metrics(ranking,t["target"],r);recon=reconstruct_metrics(t["activations"],t["down_weight"],m["indices"],t["down_bias"]);config=diagnostics["config"];row={"model":config["kind"],"latent_dim":config["latent_dim"],"stage":"fine_tuned" if config.get("init_checkpoint") else "ce","loss":config.get("loss","unknown"),"initialization":config.get("init_checkpoint"),"ranking_weight":config.get("ranking_weight"),"frozen_epochs":config.get("freeze_encoder_epochs",0),"train_retention":config.get("train_retention",.5),"method":method,"retention":r,**{k:float(v.mean()) for k,v in m.items() if k!="indices"},"ffn_cosine":float(recon["cosine_similarity"].mean()),"ffn_cosine_median":float(recon["cosine_similarity"].median()),"ffn_cosine_p01":float(recon["cosine_similarity"].quantile(.01)),"ffn_cosine_p05":float(recon["cosine_similarity"].quantile(.05)),"relative_l2":float(recon["relative_l2"].mean()),"relative_l2_median":float(recon["relative_l2"].median()),"relative_l2_p95":float(recon["relative_l2"].quantile(.95)),"relative_l2_p99":float(recon["relative_l2"].quantile(.99)),**predictor_accounting(model,t["x"].shape[1],t["target"].shape[1],r)};rows.append(row)
    if abs(r-.5)<1e-9:selections[method]=m["indices"].to(torch.int32).cpu()
 ...
 (run_dir/"per_sample.jsonl").write_text("".join(json.dumps(row)+"\n" for row in samples));torch.save(selections,run_dir/"selection_indices_050.pt");(run_dir/"metrics.json").write_text(json.dumps({"device_diagnostics":diagnostics,"rows":rows,"latency":benchmark(model,data["inputs"].float(),device)},indent=2)+"\n");return rows
