...
def ste_topk_mask(scores,retention,temperature=.5,normalize=True):
 ...
 hard=hard_topk_mask(scores,retention);k=max(1,math.ceil(scores.shape[-1]*retention));threshold=torch.topk(z,k,-1,sorted=True).values[...,-1:].detach();soft=torch.sigmoid((z-threshold)/temperature);return hard+soft-soft.detach()
