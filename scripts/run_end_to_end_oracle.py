...
def main(argv=None):
 p=argparse.ArgumentParser();p.add_argument("--model",default="Qwen/Qwen3.8-27B");p.add_argument("--input",type=Path);p.add_argument("--dataset");p.add_argument("--dataset-split",default="train");p.add_argument("--text-field",default="text");p.add_argument("--category-field",default="category");p.add_argument("--exclude-capture-metadata",type=Path);p.add_argument("--schedules",default=DEFAULT_ORDER);p.add_argument("--schedule-config",type=Path,default=Path("configs/end_to_end_schedules.yaml"));p.add_argument("--schedule-interpolation",choices=["nearest","linear"]);p.add_argument("--max-eval-tokens",type=int,default=250);p.add_argument("--device-map",default="auto");p.add_argument("--offload-folder");p.add_argument("--cache-dir");p.add_argument("--dtype",default="bfloat16");p.add_argument("--revision");p.add_argument("--output-dir",type=Path,default=Path("results/end_to_end"));p.add_argument("--force",action="store_true");p.add_argument("--generation",action="store_true");p.add_argument("--generation-prompts",type=int,default=20);p.add_argument("--max-new-tokens",type=int,default=64);a,_=p.parse_known_args(argv)
