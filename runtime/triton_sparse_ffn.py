...
if TRITON_AVAILABLE:
    ...
    @triton.jit
    def _indexed_down(activation,wd,ids,out,H:tl.constexpr,I:tl.constexpr,K:tl.constexpr,TRANSPOSED:tl.constexpr,BLOCK:tl.constexpr):
        h=tl.program_id(0);acc=0.0
        for start in range(0,K,BLOCK):
            offsets=start+tl.arange(0,BLOCK);mask=offsets<K;neurons=tl.load(ids+offsets,mask=mask,other=0);a=tl.load(activation+offsets,mask=mask,other=0.).to(tl.float32);pointer=neurons*H+h if TRANSPOSED else h*I+neurons;w=tl.load(wd+pointer,mask=mask,other=0.).to(tl.float32);acc+=tl.sum(a*w)
        tl.store(out+h,acc)


def triton_indexed_ffn(x,selected_ids,gate_weight,up_weight,down_weight,transposed_down=False):
    ...
    hidden=x.shape[-1];intermediate=gate_weight.shape[0];expected=(intermediate,hidden) if transposed_down else (hidden,intermediate)
    if tuple(down_weight.shape)!=expected:raise ValueError(f"down weight shape must be {expected} for transposed_down={transposed_down}")
    k=selected_ids.numel();activation=torch.empty(k,device=x.device,dtype=x.dtype);output=torch.empty((1,hidden),device=x.device,dtype=x.dtype);_indexed_gate_up[(k,)](x,gate_weight,up_weight,selected_ids,activation,H=hidden,K=k,BLOCK=256);_indexed_down[(hidden,)](activation,down_weight,selected_ids,output,H=hidden,I=intermediate,K=k,TRANSPOSED=transposed_down,BLOCK=256);return output
