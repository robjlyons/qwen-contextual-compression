...
__global__ void latent_kernel(const half*x,const half*ew,const half*eb,const half*f1w,const half*f1b,const half*f2w,const half*f2b,const half*lw,const half*lb,half*z,int H,float eps){__shared__ float r[32],u[32],v[32];int lane=threadIdx.x&31,warp=threadIdx.x>>5;for(int q=warp;q<32;q+=8){float s=0;for(int h=lane;h<H;h+=32)s+=__half2float(x[h])*__half2float(ew[q*H+h]);s=ws(s);if(!lane){s+=__half2float(eb[q]);r[q]=s/(1.f+expf(-s));}}__syncthreads();if(threadIdx.x==0){for(int q=0;q<32;q++){float s=__half2float(f1b[q]);for(int j=0;j<32;j++)s+=__half2float(f1w[q*32+j])*r[j];u[q]=s/(1.f+expf(-s));}float mean=0,var=0;for(int q=0;q<32;q++){float s=__half2float(f2b[q]);for(int j=0;j<32;j++)s+=__half2float(f2w[q*32+j])*u[j];v[q]=s+r[q];mean+=v[q];}mean/=32;for(int q=0;q<32;q++){float d=v[q]-mean;var+=d*d;}float inv=rsqrtf(var/32+eps);for(int q=0;q<32;q++)z[q]=__float2half((v[q]-mean)*inv*__half2float(lw[q])+__half2float(lb[q]));}}
...
class CudaSelector:
 ...
 def scores(self,x):
  ...
  if x.ndim!=2 or x.shape[0]<1:raise ValueError("selector input must have shape [batch, hidden] with batch >= 1")
  p=self.source.predictor
  def one(row):return extension.scores(row.half().contiguous(),p.encoder.weight,p.encoder.bias,p.residual_fc1.weight,p.residual_fc1.bias,p.residual_fc2.weight,p.residual_fc2.bias,p.layer_norm.weight,p.layer_norm.bias,p.neurons.weight,p.neurons.bias,p.layer_norm.eps)
  if x.shape[0]==1:return one(x)
  return torch.cat([one(x[index:index+1]) for index in range(x.shape[0])],dim=0)
