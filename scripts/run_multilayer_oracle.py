...
def main(argv=None):
 ...
 a,_=p.parse_known_args(argv);layers=[int(x) for x in a.layers.split(",")];retention=[float(x) for x in a.retention.split(",")];metadata=json.loads((a.activation_dir.parent/"metadata.json").read_text());a.output_dir.mkdir(parents=True,exist_ok=True);runs=[]
 for layer in layers:
