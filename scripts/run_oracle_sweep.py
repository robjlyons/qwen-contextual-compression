...
def main(argv=None):
 p=argparse.ArgumentParser(); p.add_argument("--model",default="Qwen/Qwen3.8-27B"); p.add_argument("--layer",type=int,default=0); p.add_argument("--activation-dir",type=Path,required=True); p.add_argument("--output",type=Path); p.add_argument("--retention",default=DEFAULT); p.add_argument("--importance",default="weighted_activation"); p.add_argument("--block-sizes",default="32,64,128,256"); p.add_argument("--random-repeats",type=int,default=3); p.add_argument("--batch-size",type=int,default=8); p.add_argument("--seed",type=int,default=42); p.add_argument("--dtype",default="bfloat16"); p.add_argument("--device-map",default="auto"); p.add_argument("--device"); p.add_argument("--offload-folder"); p.add_argument("--revision")
 a,_=p.parse_known_args(argv); output=a.output or a.activation_dir/"oracle.csv"; model=load_model(a.model,a.dtype,a.device_map,a.offload_folder,a.revision); info=inspect(model,a.layer); (output.parent/"experiment_config.json").write_text(json.dumps(vars(a)|{"output":str(output),"activation_dir":str(a.activation_dir),"inspection":info},default=str,indent=2)+"\n")
