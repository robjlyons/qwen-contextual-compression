 ...
 with torch.inference_mode():
  ...
  for r in retentions:
   for method,ranking in (("predictor",pred),("oracle",t["target"]),("static",t["static"].expand_as(t["target"]))):
    m=selection_metrics(ranking,t["target"],r);recon=reconstruct_metrics(t["activations"],t["down_weight"],m["indices"],t["down_bias"]);config=diagnostics["config"];row={"model":config["kind"],"loss":config.get("loss","unknown"),"initialization":config.get("init_checkpoint"),"ranking_weight":config.get("ranking_weight"),"frozen_epochs":config.get("freeze_encoder_epochs",0),"train_retention":config.get("train_retention",.5),"method":method,"retention":r,**{k:float(v.mean()) for k,v in m.items() if k!="indices"},"ffn_cosine":float(recon["cosine_similarity"].mean()),"ffn_cosine_p01":float(recon["cosine_similarity"].quantile(.01)),"ffn_cosine_p05":float(recon["cosine_similarity"].quantile(.05)),"relative_l2":float(recon["relative_l2"].mean()),"relative_l2_p95":float(recon["relative_l2"].quantile(.95)),"relative_l2_p99":float(recon["relative_l2"].quantile(.99)),**predictor_accounting(model,t["x"].shape[1],t["target"].shape[1],r)};rows.append(row)
